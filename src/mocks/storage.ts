 ...
 * 보유 쿠폰 목록 조회
 */
export function getMyCoupons(): Coupon[] {
	return couponDefinitions
		.map((def) => ({
			...def,
			currentOwnedCount: userCoupons.get(def.couponCode) || 0,
		}))
		.filter((coupon) => coupon.currentOwnedCount > 0)
}
...
export function claimCoupon(couponCode: string): {
	...
	return {
		...
		coupon: {
			...couponDef,
			currentOwnedCount: currentCount + 1,
		},
	}
}
