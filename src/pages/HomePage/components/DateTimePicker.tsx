...
export const DateTimePicker = ({ initialValue = new Date(), onChange }: DateTimePickerProps) => {
  const [value, setValue] = useState<DateTimeValue>(dateToPickerValue(initialValue));
  ...
  return (
    <Box className={pickerStyles}>
      <Picker value={value} onChange={handleChange} wheelMode="natural" height={250} itemHeight={50}>
        {Object.keys(selections).map(name => (
          <Picker.Column key={name} name={name}>
            {Object.keys(selections[name as keyof typeof selections]).map(option => (
              <Picker.Item key={option} value={option}>
                {({ selected }) => (
                  <div className={selected ? 'picker-item picker-item-selected' : 'picker-item'}>
...
const generateDates = () => {
  ...
  for (let i = 0; i < 365; i++) {
    ...
    const label = i === 0 ? '오늘' : i === 1 ? '내일' : `${year}.${month}.${day}`;
    dates[key] = label;
  }
